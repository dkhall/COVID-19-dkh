...
ax.annotate('China: {:,}'.format(cases_cn[-1]), (dates[-1]+dt.timedelta(days=1), 0.85*cases_cn[-1]), color='red', fontsize=9)

plt.semilogy(dates[np.where(cases_it>0)[0][0]:], cases_it[cases_it>0], 'g-')
plt.semilogy(dates[-1], cases_it[-1], 'g.')
ax.annotate('Italy: {:,}'.format(cases_it[-1]), (dates[-1]+dt.timedelta(days=1), 0.6*cases_it[-1]), color='green', fontsize=9)

plt.semilogy(dates[np.where(cases_us>0)[0][0]:], cases_us[cases_us>0], 'b-')
plt.semilogy(dates[-1], cases_us[-1], 'b.')
ax.annotate('US: {:,}'.format(cases_us[-1]), (dates[-1]+dt.timedelta(days=1), 1.25*cases_us[-1]), color='blue', fontsize=9)
